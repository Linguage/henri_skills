...
```bash
# 步骤 1-3：生成分类报告
python <skill-dir>/scripts/classify_images.py

# 也可导出 JSON 结果
python <skill-dir>/scripts/classify_images.py --json 图片/classification_report.json

# 步骤 4：确认后执行移动
python <skill-dir>/scripts/classify_images.py --move

# 仅扫描根目录（不扫描其他图片/）
python <skill-dir>/scripts/classify_images.py --scan-root

# 自定义设备白名单（EXIF 匹配用）
python <skill-dir>/scripts/classify_images.py --own-devices iPhone iPad "Xiaomi 14"
```
